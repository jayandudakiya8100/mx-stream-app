...
class OldVegaMoviesProvider {
  ...
  static Future<List<OldStreamLink>> fetchStreams(String title) async {
    ...
    final List<OldStreamLink> allStreams = [];
...
class VCloudExtractor {
  ...
  static Future<List<OldStreamLink>> extract(String url) async {
    final List<OldStreamLink> links = [];
    try {
      ...
      for (final button in buttons) {
        ...
        if (text.contains('FSL Server')) {
          links.add(OldStreamLink(name: 'V-Cloud [FSL Server]', streamUrl: btnLink, quality: qualityText));
        } else if (text.contains('FSLv2')) {
          links.add(OldStreamLink(name: 'V-Cloud [FSLv2 Server]', streamUrl: btnLink, quality: qualityText));
        } else if (text.contains('Mega Server')) {
          links.add(OldStreamLink(name: 'V-Cloud [Mega Server]', streamUrl: btnLink, quality: qualityText));
        } else if (text.contains('Download File')) {
          links.add(OldStreamLink(name: 'V-Cloud [Download File]', streamUrl: btnLink, quality: qualityText));
          ...
          try {
            ...
            if (dlink.isNotEmpty) {
              ...
              links.add(OldStreamLink(name: 'V-Cloud [BuzzServer]', streamUrl: buzzBase + dlink, quality: qualityText));
            }
          } catch (_) {
          ...
          if (pixelLink != null && pixelLink.isNotEmpty) {
            ...
            links.add(OldStreamLink(name: 'V-Cloud [Pixeldrain]', streamUrl: finalURL, quality: qualityText));
          }
          ...
          if (redirectUrl != null) {
            ...
            links.add(OldStreamLink(name: 'V-Cloud [10Gbps Server]', streamUrl: finalUrl, quality: qualityText));
          }
        }
      }
    } catch (_) {
